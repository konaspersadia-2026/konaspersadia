  ...
  const ilmiahSchedule = [
    {
      ...
      events: [
        ...
        },
        {
          ...
          description: [
            "Room 1: Update Terapi Diabetes Tipe 2 & Inovasi Obat Terkini",
            "Room 2: Epidemiologi Global & Terapi Farmakologis Obesitas",
            "Room 3: Diabetes Remisi & Teknologi Diabetes 2026 (CGM, AI)"
          ],
        ...
        },
        {
          ...
          description: [
            "Room 1: Kardiometabolik & Perlindungan Ginjal pada Diabetes",
            "Room 2: Patofisiologi Obesitas & Pendekatan Nutrisi Presisi",
            "Room 3: Diabetes Usia Lanjut & Neuropati Diabetik"
          ],
          location: "Room 1, Room 2, Room 3",
          speaker: "Berbagai Narasumber",
          type: "symposium"
        },
        {
          ...
          description: [
            "Update Terapi Diabetes dan Obesitas: Peran Agonis GLP-1/GIP",
            "Masa Depan Terapi Diabetes & Obesitas Menuju Pengobatan Personal"
          ],
        ...
        },
        {
          ...
          description: [
            "WS 1: Teknologi Diabetes Terkini (CGM, AID, Digital Health)",
            "WS 2: Manajemen Obesitas Holistik (Nutrisi, Fisik, Farmakoterapi)",
            "WS 3: Pencegahan Komplikasi Diabetes di Lini Pertama"
          ],
        ...
        },
        {
          ...
          description: [
            "Kata Sambutan, Pembacaan Hasil Penelitian, Diskusi, Drama, Lagu dan Tari Lilin-Lilin Kecil"
          ],
          location: "Novotel Bogor Convention Center",
          ...
          {activeTab === "ilmiah" ? (
                        ...
                        <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-slate-100 hover:border-[#00B4AC]/40 transition group-hover:shadow-md">
                          ...
                          {(event as any).description && (
                            <div className="mb-3 space-y-1.5">
                              {(event as any).description.map((desc: string, i: number) => (
                                <p key={i} className="text-[11px] sm:text-xs text-slate-600 leading-relaxed pl-2 border-l-2 border-[#0B3D5E]/20">
                                  {desc}
                                </p>
                              ))}
                            </div>
            ...
            <div className="space-y-12">
              {pestaSchedule.map((dayGroup, groupIdx) => (
                <div key={groupIdx} className="bg-gradient-to-br from-[#E8F0FE]/50 via-[#E6F4EA]/50 to-[#FFF8E1]/50 rounded-2xl p-6 sm:p-8 border border-[#2D7A4F]/20">
                  ...
                  <div className="space-y-6 relative border-l-2 border-[#00B4AC]/35 ml-4 sm:ml-6 pl-4 sm:pl-8">
                    {dayGroup.events.map((event, eventIdx) => (
                      <div key={eventIdx} className="relative group">
                        ...
                        <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-slate-100 hover:border-[#00B4AC]/40 transition group-hover:shadow-md">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                            <span className="flex items-center gap-1 text-xs font-bold text-text-[#2D7A4F] bg-[#E6F4EA] px-2 py-1 rounded">
                              <Clock className="h-3.5 w-3.5" />
                              {event.time}
                            </span>
                            <span className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${getBadgeStyle(event.type)}`}>
                          ...
                          {(event as any).description && (
                            <div className="mb-3 space-y-1.5">
                              {(event as any).description.map((desc: string, i: number) => (
                                <p key={i} className="text-[11px] sm:text-xs text-slate-600 leading-relaxed pl-2 border-l-2 border-[#2D7A4F]/20">
                                  {desc}
                                </p>
                              ))}
                            </div>
        ...
        <div className="max-w-3xl mx-auto mt-12 p-4 bg-amber-50 border border-amber-200/60 rounded-xl flex items-start gap-3">
          ...
          <div>
            <p className="text-xs sm:text-sm text-amber-800 leading-relaxed font-medium">
              *Jadwal dan topik ilmiah di atas sewaktu-waktu dapat berubah menyesuaikan dengan kondisi di lapangan. Informasi perubahan terbaru akan selalu kami perbarui melalui website ini dan pengumuman resmi.
            </p>
          </div>
        </div>
