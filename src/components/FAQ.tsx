...
export default function FAQ() {
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: "Setelah mengisi biodata diri dan memilih kategori peserta di pop-up pendaftaran, sistem kami akan memunculkan nominal tagihan transfer yang ditambahkan dengan 3 digit kode unik acak (misal: Rp 1.500.342). Silakan transfer ke rekening resmi panitia tepat pas hingga digit terakhir tersebut. Anda tidak perlu mengunggah bukti transfer, karena tim bendahara kami akan mencocokkan mutasi rekening berdasarkan 3 digit kode unik tersebut secara otomatis dan memperbarui status pendaftaran Anda."
    },
    {
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          {faqs.map((faq, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-slate-50 rounded-2xl border border-slate-200/40 overflow-hidden transition-all duration-200 hover:border-[#00B4AC]/40 shadow-sm"
              >
                <button
                ...
                >
                  <span className="font-extrabold text-slate-800 text-sm sm:text-base flex items-center gap-2.5">
                    <HelpCircle className="h-5 w-5 text-[#00B4AC] shrink-0" />
                    {faq.q}
                  </span>
                  {isOpen ? (
                    <ChevronUp className="h-5 w-5 text-[#0B3D5E] shrink-0" />
                  ) : (
